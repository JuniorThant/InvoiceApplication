...
import type {API} from '../../api'
  import { createInvoiceService, getAllInvoiceService, getInvoiceByNumber } from '../../services/invoice'
  ...
  import { onMount } from 'svelte'
   ...
   token=localStorage.getItem("token")
    ...
    onMount(()=>{
      const generateInvoiceId=async(id:string)=>{
        if(!token) return;
      ...
      const fetchInvoice=async()=>{
        if(!token) return;
        const data=await  getInvoiceByNumber(token,invoiceId)
      }
    })
